...
function app(storage=new Map()){
 ...
 const utterances=[];const timers=[];const timerDelays=[];
 class Recognition{start(){this.onstart?.()}stop(){this.onend?.()}abort(){}}
 const context=vm.createContext({console,URL,Blob,window:{SpeechRecognition:Recognition,speechSynthesis:{cancel(){},getVoices(){return [{lang:'fr-FR'},{lang:'de-DE'}]},speak(u){utterances.push(u)}}},document:{querySelector:get,querySelectorAll:()=>[],addEventListener(event,fn){(listeners[event]??=[]).push(fn)},documentElement:{classList:{toggle(){}}}},navigator:{},localStorage:{getItem(k){return storage.get(k)??null},setItem(k,v){storage.set(k,v)}},setTimeout(fn,delay){timers.push(fn);timerDelays.push(delay);return timers.length},clearTimeout(){},SpeechSynthesisUtterance:class{constructor(t){this.text=t}}});
 ...
 return {run:s=>vm.runInContext(s,context),get,utterances,timers,timerDelays,emit(event){listeners[event]?.forEach(fn=>fn({}));},click(dataset){const button={dataset,classList:{add(){}}};listeners.click.forEach(fn=>fn({target:{closest:()=>button}}))}};
}
...
test('pending microphone permission cannot start recording in another lesson',async()=>{
 const a=app();a.run("var released=0,created=0;var resolveMic; navigator.mediaDevices={getUserMedia:()=>new Promise(resolve=>resolveMic=resolve)};window.MediaRecorder=class{constructor(){created++}};openLesson('fr',0);step=3;lessonRender();var pendingRecording=record();openLesson('de',0);step=3;lessonRender();resolveMic({getTracks:()=>[{stop(){released++}}]})");
 await a.run('pendingRecording');assert.equal(a.run('created'),0);assert.equal(a.run('released'),1);assert.equal(a.run('recordingPending'),false);
});
test('recorder constructor failure releases the microphone and permits retry',async()=>{
 const a=app();a.run("var released=0;navigator.mediaDevices={getUserMedia:async()=>({getTracks:()=>[{stop(){released++}}]})};window.MediaRecorder=class{constructor(){throw Error('unavailable')}};openLesson('fr',0);step=3;lessonRender()");await a.run('record()');assert.ok(a.run('released')>=1);assert.equal(a.run('recordingPending'),false);assert.equal(a.get('#record-button').disabled,false);
});
test('stale recording callbacks cannot stop a newer microphone stream or show its audio',async()=>{
 const a=app();a.run("var stopped=[],sequence=0; navigator.mediaDevices={getUserMedia:async()=>{const id=++sequence;return {getTracks:()=>[{stop(){stopped.push(id)}}]}}};window.MediaRecorder=class{constructor(){this.state='inactive';this.mimeType='audio/webm'}start(){this.state='recording'}stop(){this.state='inactive'}};openLesson('fr',0);step=3;lessonRender()");await a.run('record()');a.run("var lateStop=recorder.onstop;openLesson('de',0);step=3;lessonRender()");await a.run('record()');a.run('lateStop()');assert.equal(a.run('stopped.includes(2)'),false);assert.equal(a.run("recorder.state==='recording'"),true);assert.equal(a.run('recordUrl'),null);
});
test('speech errors and a missing end event recover without passing the phrase',()=>{
 const a=app();a.run("openLesson('fr',0);step=3;lessonRender();checkSpeech();speechCheck.onerror({error:'not-allowed'})");assert.equal(a.run('speechCheck'),null);assert.equal(a.run('speechPassed.size'),0);assert.match(a.get('#speech-feedback').textContent,/denied/);
 a.run('checkSpeech();speechCheck.stop=()=>{};stopSpeechWithResult()');a.timers.at(-1)();assert.equal(a.run('speechCheck'),null);assert.equal(a.get('#check-speech').disabled,false);assert.equal(a.run('speechPassed.size'),0);assert.match(a.get('#speech-feedback').textContent,/did not finish/);
});
test('hiding a page cancels microphone work without losing a checked phrase',()=>{
 const a=app();a.run("openLesson('fr',0);step=3;speechPassed.add(0);lessonRender();checkSpeech();var lateResult=speechCheck.onresult;document.hidden=true");a.emit('visibilitychange');assert.equal(a.run('speechCheck'),null);
 a.run("lateResult({resultIndex:0,results:[[{transcript:active.d.phrases[0][0]}]]})");assert.equal(a.run('speechPassed.size'),1);assert.equal(a.run("resumePoint('fr').speechPassed.length"),1);
});

test('recording produces replay audio, cannot play a model during capture, and never passes speech',async()=>{
 const a=app();a.run("var stops=0; navigator.mediaDevices={getUserMedia:async()=>({getTracks:()=>[{stop(){stops++}}]})};window.MediaRecorder=class{constructor(){this.state='inactive';this.mimeType='audio/webm'}start(){this.state='recording'}stop(){this.state='inactive';this.ondataavailable({data:new Blob(['sample'])});this.onstop()}};openLesson('fr',0);step=3;lessonRender()");
 await a.run('record()');a.run("playModel('Bonjour','fr',false)");assert.equal(a.utterances.length,0);
 await a.run('record()');assert.equal(a.get('#playback').hidden,false);assert.ok(a.get('#playback').src.startsWith('blob:'));assert.equal(a.run('speechPassed.size'),0);assert.equal(a.get('#record-button').disabled,false);assert.equal(a.run('stops'),1);
 a.run('stopRecording()');assert.equal(a.run('recordUrl'),null);assert.equal(a.get('#playback').hidden,true);
});
test('the longest course phrase gets at least 20 seconds and the stop watchdog ignores old checks',()=>{
 const a=app();a.run("var longest=Object.entries(COURSE).flatMap(([l,c])=>c.lessons.flatMap((d,i)=>d.phrases.map((p,j)=>({l,i,j,words:p[0].split(' ').length})))).sort((a,b)=>b.words-a.words)[0];openLesson(longest.l,longest.i);step=3;speakingIndex=longest.j;lessonRender();checkSpeech()");assert.ok(a.timerDelays.at(-1)>=20000);assert.ok(a.timerDelays.at(-1)<=60000);
 a.run('speechCheck.stop=()=>{};stopSpeechWithResult()');const oldTimeout=a.timers.at(-1);a.run('cancelSpeechCheck();checkSpeech()');oldTimeout();assert.notEqual(a.run('speechCheck'),null);a.run('cancelSpeechCheck()');
});
