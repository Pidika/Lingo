...
function app(storage=new Map()){
 ...
 for(const name of ['lessons','hints','month-one','month-two','month-three','lesson-activities','speech','app'])vm.runInContext(fs.readFileSync('dist/'+name+'.js','utf8'),context);
...
test('both courses contain 90 complete lessons with valid answers and fifteen checkpoints',()=>{
 const a=app();
 for(const l of ['fr','de']){
  assert.equal(a.run(`COURSE.${l}.lessons.length`),90);
  assert.equal(a.run(`COURSE.${l}.lessons.filter(d=>d.checkpoint).length`),15);
  ...
  for(let i=0;i<90;i++)for(let step=0;step<4;step++)a.run(`openLesson('${l}',${i});step=${step};lessonRender()`);
 }
});
...
test('old seven-day backups and new day-sixty backups restore without dropping progress',async()=>{
 ...
 a.run("state.done.fr=Array.from({length:90},(_,i)=>i);render()");assert.match(a.get('#main').innerHTML,/AVAILABLE LESSONS COMPLETE/);
});
...
test('finishing the first month opens day 31 and day 91 is unavailable',()=>{
 ...
 a.run("openLesson('de',90)");assert.equal(a.run('active.i'),59);assert.match(a.get('#notice').textContent,/not available/);
});
