...
function app(){
 ...
 for(const name of ['lessons','hints','month-one','lesson-activities','speech','app'])vm.runInContext(fs.readFileSync('dist/'+name+'.js','utf8'),context);
...
test('both courses contain 30 complete lessons with valid answers and five checkpoints',()=>{
 const a=app();
 for(const l of ['fr','de']){
  assert.equal(a.run(`COURSE.${l}.lessons.length`),30);
  assert.equal(a.run(`COURSE.${l}.lessons.filter(d=>d.checkpoint).length`),5);
  ...
  for(let i=0;i<30;i++)for(let step=0;step<4;step++)a.run(`openLesson('${l}',${i});step=${step};lessonRender()`);
 }
});
...
test('old seven-day backups and new day-thirty backups restore without dropping progress',async()=>{
 const a=app();for(const done of [[0,6],[0,6,14,29]]){
 ...
 a.run("state.done.fr=Array.from({length:30},(_,i)=>i);render()");assert.match(a.get('#main').innerHTML,/MONTH ONE COMPLETE/);
});
